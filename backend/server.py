...
# ---------- EMAIL PREFERENCES (opt-in transcript) ----------

@api.get("/me/email-prefs")
...
# ---------- CLOSE CHAT SESSION (writes narrative + optional email) ----------

@api.post("/projects/{project_id}/chronicle/close-session")
async def close_chat_session(
    ...
    # Pull message history (user + assistant turns)
    ...
    # Build a condensed timeline string for the chronicle prompt
    timeline = []
    for m in msgs[:60]:
    ...
    narrative_text = await _chronicle_narrative(
    ...
    # _chronicle_narrative wrote the entry; recover the body for email
