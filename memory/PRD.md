...
- **Resizable IDE panels (2026-06-25)**: file-tree ↔ editor-area ↔ AI panel via `react-resizable-panels` v4 (`Group` / `Panel` / `Separator`). Terminal height controlled by a custom drag handle (the inner-nested vertical PanelGroup hit a known v4 layout bug — w=0 separator — so swapped for a state-driven flex split with `localStorage` persistence). Hover cyan grip rail at the editor↔terminal divider.
